...
export default function LocationsPage() {
  ...
  return (
    <div className="wl-page">
      <header className="wl-page-hero"><h1>Find your table.</h1><p>Choose the {tenant.name} location that fits your plans.</p></header>
      ...
      <section className="wl-location-detail">
        ...
        <div className="wl-location-copy">
          <span className="wl-kicker">{selected.cuisineName}</span>
          ...
          <Link className="wl-button" to={`${tenantPath("reserve")}?branch=${selected.id}`}>Reserve at {branchShortName(selected)}</Link>
        </div>
      </section>
    </div>
  );
